...
import microservices.practice.demo.config.ElasticQueryConfigData;
...
import microservices.practice.elastic.query.util.ElasticQueryUtil;
...
public class TwitterElasticQueryClient implements ElasticQueryClient<TwitterIndexModel> {
    ...
    public TwitterIndexModel getIndexModelById(String id) {
        ...
        SearchHit<TwitterIndexModel> searchResult = elasticsearchOperations.searchOne(query, TwitterIndexModel.class,
                IndexCoordinates.of(elasticConfigData.getIndexName())); //Can search using a Query object by which complex queries can be sent to elasticsearch
        if (searchResult == null) {
            LOG.error("No document found elasticsearch with id {}", id);
